...
# # PARA SELECAO DAS ESTACOES
# postos = pd.read_excel('inventario_banco_simepar_jul2021.xlsx')
# t_ini = dt.datetime(2020,1,1)
# t_fim = dt.datetime(2021,1,1)

# ORGAOS JA COLETADOS
# orgaos1 = ['Simepar', 'Inmet']

# ORGAOS QUE FALTAM COLETAR
# orgaos = ['SAISP', 'CEMADEN', 'PETROBRAS', 'ELEJOR',
...
postos = pd.read_csv(f'info_{orgao}.csv', index_col=0)
t_ini = dt.datetime(1990,1,1)
t_fim = dt.datetime.now()

# Selecao dos postos por orgao, ou apenas reindex p/ coleta
...
for idx, row in postos_orgao.iterrows():
    ...
    if (inicio is None) or (dados_dia['t_med'].first_valid_index() < inicio):
...
# # APENAS PARA SELECAO DAS ESTACOES
# #exporta df com dados das estacoes com dados
# dfselecao = dfselecao.reset_index()
