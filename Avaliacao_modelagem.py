...
data_ini = dt.datetime(2018, 8, 22,  15,  0) #YYYY, M, D, H, MinB
data_fim = dt.datetime(2018, 9, 5,  5,  59)
...
for nome_bacia, modelos in calibracoes_usinas.items():
    ...
    serie_observada['Hora'] = serie_observada[0].str.slice(11,13).astype(
        str).astype(int)
    serie_observada['q_m3s'] = pd.to_numeric(serie_observada[0].str.slice(
        14,22), errors = 'coerce')
    serie_observada['Data'] = pd.to_datetime(serie_observada[
        ['Year', 'Month', 'Day']]) + pd.to_timedelta(serie_observada['Hora'],
                                                     unit = 'h')
...
#ANALISE DE CALIBRACOES PARA HORIZONTES
for horizonte in [23, 47, 71]:
    plt.figure()
    plt.plot(serie_observada['q_m3s'], label = "Observado", linewidth = 0.8)
    plt.title(nome_bacia + ' - Previsao ' + str(horizonte+1) + ' horas',loc='left')
    plt.xlabel('Data')
    plt.ylabel('Q [m3s-1]')
    plt.annotate(data_texto, xy=(1,0), xytext=(-4,25), fontsize=10,
                 xycoords='axes fraction', textcoords='offset points',
                 bbox=dict(facecolor='white', alpha=0.8),
                 horizontalalignment='right', verticalalignment='top')
    plt.savefig(bacia+'_observado_'+data_ini.strftime("%Y%m%d")+"_"+
                          data_fim.strftime("%Y%m%d")+'.png', dpi = 300)
    for modelo in ['916', '917']:
        if modelo == '916':
            modelagem = 'Sacramento'
        if modelo == '917':
            modelagem = 'RRR'
        plt.plot(gbl["avaliacao_"+modelo+"_"+str(horizonte)]['Vazao_prev'],
                 label = modelagem, linewidth = 0.7)
    plt.legend(loc='best')
    plt.savefig(bacia+'_comparacao_'+str(horizonte+1)+'.png', dpi = 300)
    plt.close()
