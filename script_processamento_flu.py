...
for filename in estacoes:
    cod = filename
    lista.append(cod)

    # cria DFs padrão de data, para serem preenchidas com os dados baixados
    date_rng_horario = pd.date_range(start='01/01/2020', end='26/08/2020', freq='H')
    ...
    date_rng_diario = pd.date_range(start='01/01/2020', end='26/08/2020', freq='D')
    ...
    df = pd.read_csv(cod+'_telemetrica.csv', sep=",")
    df.columns = ['data', cod, 'sensor']
    ...
    df_horario.loc[df_horario['count'] < 2, [cod]] = np.nan
    ...
    df_diario.loc[df_diario['count'] < 12, [cod]] = np.nan
    ...
    table_dia.to_csv(cod+'_telemetrica.csv')
