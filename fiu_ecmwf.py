...
for N in range(1, grbs.messages +1):
    data, lats, lons = grbs[N].data(lat1=miny, lat2=maxy, lon1=minx, lon2=maxx)
    p = np.nanmean(data) #media da precipitacao entre os pontos
    if np.isnan(p):
        p = 0
    df.loc[horizonte,N] = p

## Esse resultado retorna então o horizonte 1 (1 hora a frente do disparo)
## 102 tentativas de ensemble são tomadas
## Em vários casos, retornam valores nulos
df
